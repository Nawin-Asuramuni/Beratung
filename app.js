...
window.confirmBusinessSlot = async function(slotId) {
  ...
  document.getElementById('bn_confirm_btn').onclick = async () => {
    ...
    if (sendConfirm) {
      if (contactObj.email) {
        // Bestätigung wird NICHT sofort verschickt, sondern als Flag für den
        // automatischen Versand genau 1 Tag vor dem Termin gespeichert.
        try {
          const q2 = await getDocs(query(collection(db, 'callSlots'), orderBy('datetime', 'desc')));
          const slotDoc = q2.docs.find(d => d.id === slotId);
          if (slotDoc) await updateDoc(doc(db, 'callSlots', slotDoc.id), { sendConfirmEmail: true });
        } catch {}
        toast('📬 Bestätigung wird 1 Tag vor dem Termin automatisch per E-Mail versendet.', 'success');
    ...
    if (sendNotify && contactObj.email) {
      // Terminbenachrichtigung = sofortige E-Mail mit Benachrichtigungs-Vorlage
      // UND echter Kalender-Einladung (.ics, METHOD:REQUEST). Gmail/Outlook/Apple
      // Mail erkennen das automatisch als Termineinladung und zeigen direkt in
      // der Mail "Ja/Nein/Vielleicht"-Buttons – ein Klick trägt den Termin sofort
      // in den Kalender des Empfängers ein, wie bei Calendly.
      await sendConfirmationEmail(contactObj, { ...slotObj, mailType: 'notify', withIcs: true });
    }
  };
};
