...
// ── Vorlagen-State (localStorage-backed, live mit Firestore synchronisiert) ──
...
// Mapping Textarea-ID (Tracker-UI) -> Feldname im Firestore-Dokument settings/kaVorlagen.
// WICHTIG: Diese Feldnamen müssen 1:1 mit DEFAULT_TEMPLATES in content.js übereinstimmen,
// da die Extension (background.js) exakt dieselben Feldnamen aus Firestore ausliest.
const KA_VORLAGE_FIELD_MAP = {
...
async function kaWriteVorlageToFirestore(field, value) {
  // setDoc mit merge:true, damit das Dokument settings/kaVorlagen bei Bedarf
  // automatisch angelegt wird und andere Felder nicht überschrieben werden.
  const { setDoc } = await import('https://www.gstatic.com/firebasejs/11.3.1/firebase-firestore.js');
  await setDoc(doc(kaDb, 'settings', 'kaVorlagen'), { [field]: value }, { merge: true });
}

window.kaSaveVorlage = async function(id) {
  const el = document.getElementById(id);
  if (!el) return;
  const field = KA_VORLAGE_FIELD_MAP[id];
  try {
    localStorage.setItem('kaVorlage_' + id, el.value);
    if (field) await kaWriteVorlageToFirestore(field, el.value);
    window.toast?.('Vorlage gespeichert · wird live in Kleinanzeigen verwendet ✓', 'success');
  } catch (e) {
    window.toast?.('Speichern fehlgeschlagen: ' + e.message, 'error');
  }
};

window.kaResetVorlage = async function(id) {
  const field = KA_VORLAGE_FIELD_MAP[id];
  const def   = KA_VORLAGEN_DEFAULTS[id] || '';
  try {
    localStorage.removeItem('kaVorlage_' + id);
    const el = document.getElementById(id);
    if (el) el.value = def;
    if (field) await kaWriteVorlageToFirestore(field, def);
...
// Live-Sync: hält den lokalen Cache (localStorage) synchron mit Firestore,
// egal ob die Änderung hier im Tracker oder anderswo ausgelöst wurde.
let kaVorlagenUnsubscribe = null;
function kaInitVorlagenSync() {
  try {
    kaVorlagenUnsubscribe = onSnapshot(doc(kaDb, 'settings', 'kaVorlagen'), snap => {
      const data = snap.data() || {};
      let changed = false;
      for (const [id, field] of Object.entries(KA_VORLAGE_FIELD_MAP)) {
        if (data[field] === undefined) continue;
        if (localStorage.getItem('kaVorlage_' + id) !== data[field]) {
          localStorage.setItem('kaVorlage_' + id, data[field]);
          changed = true;
        }
      }
      if (changed && document.querySelector('.nav-item.active[data-page="kleinanzeigen"]')) {
        renderKleinanzeigen();
      }
    }, err => console.warn('[KA-Vorlagen] Sync-Fehler:', err.message));
  } catch (e) {
    console.warn('[KA-Vorlagen] Init fehlgeschlagen:', e.message);
  }
}
