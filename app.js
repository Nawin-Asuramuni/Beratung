  ...
  const kanalBlocks = KA_SOCIAL_CHANNELS.map(k => {
    ...
    const msgCards = !checked ? '' : msgs.length === 0
      ...
      : msgs.map(msg => {
          ...
          const verlauf = Array.isArray(msg.messages) && msg.messages.length > 0
            ? msg.messages.map(m => {
                const isMe = m.sender === 'ich';
                return `<div style="display:flex;justify-content:${isMe ? 'flex-end' : 'flex-start'};margin-bottom:6px;">
                  <div style="max-width:75%;padding:8px 12px;border-radius:${isMe ? '16px 16px 4px 16px' : '16px 16px 16px 4px'};
                    background:${isMe ? col.c : 'var(--bg3,var(--card-bg))'};
            ...
            : `<div style="font-size:13px;color:var(--text3,var(--text-muted));padding:8px 0;">"${msg.text}"</div>`;

          const verlaufId = 'verlauf_' + msg.id;
          const displayName = msg.contactName && msg.contactName !== msg.profilLink
            ? msg.contactName
