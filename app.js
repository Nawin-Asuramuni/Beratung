...
import {
  ...
  updateDoc, deleteDoc, onSnapshot, orderBy, query, where, serverTimestamp, setDoc, getDoc
...
// Gmail-Versand & Kalender-Zugriff direkt beim Login mit anfordern,
// damit nur noch EIN Google-Popup nötig ist statt mehrerer separater.
...
// CONTACTS PAGE
// ============================================================
function renderContacts(filter = '', statusFilter = '', catFilter = '', futureEmpFilter = false) {
  ...
  const filtered = contacts.filter(c => {
    ...
    const matchFutureEmp = !futureEmpFilter || c.futureEmployee === true;
    return matchSearch && matchStatus && matchCat && matchFutureEmp;
  });

  const futureEmpCount = contacts.filter(c => c.futureEmployee === true).length;
  ...
  document.getElementById('content').innerHTML = `
    ...
    <div style="display:flex;gap:6px;margin-bottom:14px;flex-wrap:wrap;align-items:center;">
      <button onclick="renderContacts(document.getElementById('searchInput')?.value||'',document.getElementById('statusFilter')?.value||'','',${futureEmpFilter})"
      ...
      ${Object.entries(labels).map(([key, lbl]) => `
        <button onclick="renderContacts(document.getElementById('searchInput')?.value||'',document.getElementById('statusFilter')?.value||'','${key}',${futureEmpFilter})"
          style="${tabStyle(key)}">
      ...
      <button onclick="renderContacts(document.getElementById('searchInput')?.value||'',document.getElementById('statusFilter')?.value||'','${catFilter}',${!futureEmpFilter})"
        style="font-size:13px;padding:6px 14px;border-radius:20px;cursor:pointer;transition:all .15s;
        border:1.5px solid ${futureEmpFilter?'#eab308':'var(--border2)'};
        background:${futureEmpFilter?'rgba(234,179,8,0.12)':'transparent'};
        color:${futureEmpFilter?'#eab308':'var(--text2)'};
        font-weight:${futureEmpFilter?'600':'400'};">
        ⭐ Potenzielle Mitarbeiter <span style="font-size:11px;opacity:.7;">(${futureEmpCount})</span>
      </button>
    </div>
      ...
      : `<div class="contacts-table-wrap">
          <table class="contacts-table">
            ...
            <tbody>
              ...
              <tr>
                <td>
                  <div class="contact-name">${c.vorname} ${c.nachname}${c.futureEmployee ? ' <span title="Potenzielle/r Mitarbeiter:in" style="font-size:12px;">⭐</span>' : ''}</div>
  ...
  document.getElementById('searchInput').addEventListener('input', e => renderContacts(e.target.value, document.getElementById('statusFilter').value, catFilter, futureEmpFilter));
  document.getElementById('statusFilter').addEventListener('change', e => renderContacts(document.getElementById('searchInput').value, e.target.value, catFilter, futureEmpFilter));
}
...
window.showContact = function(id) {
  ...
  document.getElementById('content').innerHTML = `
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:10px;">
      <div>
        <h2 style="font-size:20px;font-weight:600;">${c.vorname} ${c.nachname}${c.futureEmployee ? ' <span title="Potenzielle/r Mitarbeiter:in" style="font-size:16px;">⭐</span>' : ''}</h2>
      ...
      <div>
        <div class="detail-card" style="margin-bottom:16px;">
          ...
          <div class="detail-row">
            <span class="detail-key">⭐ Potenz. Mitarbeiter</span>
            <span class="detail-val">
              <label style="display:flex;align-items:center;gap:6px;cursor:pointer;">
                <input type="checkbox" ${c.futureEmployee ? 'checked' : ''} onchange="setFutureEmployee('${c.id}', this.checked)" style="width:15px;height:15px;cursor:pointer;">
                <span style="font-size:13px;color:var(--text2);">${c.futureEmployee ? 'Ja' : 'Nein'}</span>
              </label>
            </span>
          </div>
        </div>
...
window.openContactModal = function(id = null) {
  ...
  const futureEmpChk = document.getElementById('f_futureEmployee');
  if (futureEmpChk) futureEmpChk.checked = c?.futureEmployee || false;
...
document.getElementById('saveContact').addEventListener('click', async () => {
  ...
  const data = {
    ...
    futureEmployee: document.getElementById('f_futureEmployee')?.checked || false,
  ...
  try {
    ...
    if (wvDate && wvType && contactId && !contactId.startsWith('local_')) {
      ...
      const slotDoc = {
        ...
        apptType: wvType,
        apptDuration,
        sendConfirmEmail: data.wvConfirm,
        sendReminder: data.wvReminder,
      };
      const slotRef = await addDoc(collection(db, 'callSlots'), slotDoc).catch(() => null);
      if (slotRef) slotDoc.id = slotRef.id;
      ...
      // Bestätigung wird NICHT sofort verschickt, sondern automatisch
      // 1 Tag vor dem Termin (sendConfirmEmail-Flag oben, siehe GitHub-Actions-Skript).
      if (data.wvConfirm && data.email) {
        toast('📬 Bestätigung wird 1 Tag vor dem Termin automatisch per E-Mail versendet.', 'success');
      }
    }
...
// Google-Kalender "Termin hinzufügen"-Link erzeugen (für Klick-Button in E-Mails,
// im Gegensatz zur .ics-Datei funktioniert das ohne Anhang/Download – ein Klick
// öffnet direkt den vorausgefüllten Google-Kalender-Eintrag zum Speichern).
function buildGoogleCalendarLink(contact, slotData) {
  const dt = new Date(slotData.datetime);
  const dtEnd = new Date(dt.getTime() + (slotData.apptDuration || 30) * 60000);
  const pad = n => String(n).padStart(2, '0');
  const fmt = d => `${d.getUTCFullYear()}${pad(d.getUTCMonth()+1)}${pad(d.getUTCDate())}T${pad(d.getUTCHours())}${pad(d.getUTCMinutes())}00Z`;

  const title = encodeURIComponent(`${slotData.apptType || 'Termin'}: ${contact.vorname} ${contact.nachname || ''}`.trim());
  const dates = `${fmt(dt)}/${fmt(dtEnd)}`;
  const details = encodeURIComponent(slotData.note || '');
...
async function getGmailToken() {
  ...
  // Gecachten Token verwenden, wenn noch > 2 Minuten gültig (z.B. vom Login mitgebracht)
  try {
  ...
  const doRequest = async (allowPopup = true) => {
    // Erst lautlos versuchen (kein Pop-up)
    const silentToken = await requestToken('none');
    if (silentToken) return silentToken;
    // Pop-up nur erlauben wenn explizit angefordert (d.h. durch direkten User-Klick)
    if (allowPopup) return await requestToken('');
    return null;
  };
...
function buildGmailMessage(to, toName, from, subject, htmlBody, icsContent, inlineImage) {
  ...
  if (icsContent) {
    ...
    parts.push(
      `--${outerBoundary}`,
      // "inline" statt "attachment": Gmail/Outlook erkennen die .ics als echte
      // Termineinladung und zeigen direkt in der Mail einen "In Kalender eintragen"-
      // Button – ohne dass der Empfänger irgendetwas herunterladen muss.
...
async function sendConfirmationEmail(contact, slotData) {
  ...
  const isReminder = slotData.isReminder === true || slotData.mailType === 'reminder';
  const isNotify = !isReminder && (slotData.mailType === 'notify');
...
{signatur}`)
  ...
  // Google-Kalender-Link für Klick-Button (funktioniert ohne .ics-Anhang/Download)
  const gcalUrl = buildGoogleCalendarLink(contact, slotData);
  const gcalButtonHtml = `<div style="margin:20px 0;"><a href="${gcalUrl}" target="_blank" style="background:#4285f4;color:#ffffff;text-decoration:none;padding:10px 20px;border-radius:6px;font-family:Arial,sans-serif;font-size:14px;display:inline-block;">📅 Termin zu Google Kalender hinzufügen</a></div>`;
  ...
  // {kalenderlink} durch reine URL ersetzen (für Plain-Text bzw. eigene Verlinkung in der Vorlage)
  const htmlBodyRaw = bodyText
    ...
    .replace(/{kalenderlink}/g, gcalUrl)
    .replace(/\n/g, '<br>');

  // Falls die Vorlage den Platzhalter {kalenderlink} nicht selbst nutzt, wird der
  // Button automatisch ergänzt – bei der Benachrichtigung (kein .ics-Anhang) ist
  // das der einzige Weg, den Termin direkt in den Kalender zu bekommen.
  const hasOwnLinkPlaceholder = bodyText.includes('{kalenderlink}');
  const htmlBody = hasOwnLinkPlaceholder ? htmlBodyRaw : htmlBodyRaw + gcalButtonHtml;
  ...
  if (!token) {
    // Token abgelaufen oder nicht vorhanden → einmalig Pop-up anbieten
    const reloginToast = document.createElement('div');
    reloginToast.innerHTML = `
      <span>⚠️ Gmail-Session abgelaufen.</span>
      <button onclick="window._reloginGmail && window._reloginGmail()" style="margin-left:12px;padding:4px 10px;border-radius:6px;border:1px solid #fff;background:transparent;color:#fff;cursor:pointer;font-size:12px;">Jetzt neu verbinden</button>
    `;
    reloginToast.style.cssText = 'position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:#dc2626;color:#fff;padding:12px 20px;border-radius:12px;z-index:9999;display:flex;align-items:center;gap:8px;box-shadow:0 4px 20px rgba(0,0,0,0.3);';
    document.body.appendChild(reloginToast);
    setTimeout(() => reloginToast.remove(), 8000);

    window._reloginGmail = async () => {
      reloginToast.remove();
      localStorage.removeItem('gmailOAuthToken');
      const newToken = await getGmailToken();
      if (newToken) toast('✅ Gmail erfolgreich neu verbunden. Bitte nochmal senden.', 'success');
      else toast('❌ Gmail-Login fehlgeschlagen. Seite neu laden und erneut versuchen.', 'error');
    };

    const plainBody = hasOwnLinkPlaceholder
      ? bodyText.replace(/{kalenderlink}/g, gcalUrl)
      : `${bodyText}\n\n📅 Termin zu Google Kalender hinzufügen:\n${gcalUrl}`;
    const mailtoUrl = `mailto:${contact.email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(plainBody)}`;
  ...
  try {
    ...
    if (res.ok) {
      const label = isReminder ? 'Terminerinnerung' : (isNotify ? 'Terminbenachrichtigung' : 'Terminbestätigung');
      toast(`✅ ${label} an ${contact.vorname} gesendet${slotData.withIcs !== false ? ' – inkl. Kalender-Einladung!' : '!'}`, 'success');
    } else {
...
async function syncToGoogleCalendar(slotData, contactData) {
  ...
  if (successCount > 0) {
    ...
    if (Object.keys(gcalEventIds).length > 0 && slotData.id) {
      try {
        await updateDoc(doc(db, 'callSlots', slotData.id), { gcalEventIds });
...
document.getElementById('saveCallSlot').addEventListener('click', async () => {
  ...
  const data = {
    ...
    apptColor,
    note,
    sendConfirmEmail: document.getElementById('cs_confirm').checked,
...
function showEmailDialog(contact, slotData) {
      ...
      <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:20px;">
        ...
        <label style="display:flex;align-items:flex-start;gap:12px;cursor:${hasEmail?'pointer':'not-allowed'};
          ...
          <div>
            ...
            <div style="font-size:12px;color:var(--text3);margin-top:2px;">Wird automatisch 24 Std. vor dem Termin über Gmail gesendet – inkl. Kalender-Datei (.ics)</div>
          </div>
        </label>
  ...
  document.getElementById('ed_send').addEventListener('click', async () => {
    ...
    // Terminbestätigung & Erinnerung NICHT sofort verschicken, sondern nur
    // als Flag speichern – der automatische Versand (24h vorher / Termintag 9 Uhr)
    // übernimmt das GitHub-Actions-Skript (send-scheduled-emails.js).
    try {
      const flagUpdate = {};
      if (sendConfirm)  flagUpdate.sendConfirmEmail = true;
      if (sendReminder) flagUpdate.sendReminder     = true;
      if (slotData.id && Object.keys(flagUpdate).length > 0) {
        await updateDoc(doc(db, 'callSlots', slotData.id), flagUpdate);
      }
    } catch(fe) { console.warn('Flag-Update fehlgeschlagen:', fe.message); }

    // Nur die reine .ics-Kalendereinladung geht direkt raus (ohne Bestätigungstext),
    // falls gewünscht und die Bestätigung selbst noch nicht sofort verschickt wird.
    if (sendIcs && !sendConfirm) {
      await sendCalendarInviteOnly(contact, slotData);
    }

    if (sendConfirm)  toast('📬 Bestätigung wird 24 Std. vor dem Termin automatisch per E-Mail versendet.', 'success');
    if (sendReminder) toast('🔔 Erinnerung wird am Termintag um 09:00 Uhr versendet.', 'success');
  });
}
...
{signatur}`;
  ...
  document.getElementById('content').innerHTML = `
    <style>
      ...
      .settings-section > h3 {
        cursor: pointer; user-select: none; display: flex; align-items: center; flex-wrap: wrap;
      }
      .settings-section > h3::after {
        content: ""; margin-left: auto; width: 8px; height: 8px; flex-shrink: 0;
        border-right: 2px solid currentColor; border-bottom: 2px solid currentColor;
        opacity: 0.45; transform: rotate(45deg); transition: transform .2s ease;
      }
      .settings-section.collapsed > h3::after { transform: rotate(-45deg); }
      .settings-section.collapsed > *:not(h3) { display: none !important; }
      .settings-section.collapsed { cursor: pointer; }
      .settings-section > h3:hover::after { opacity: 0.9; }
      .settings-subsection { padding-top:18px;margin-top:18px;border-top:1px solid rgba(255,255,255,0.07); }
      .settings-subsection:first-of-type { padding-top:0;margin-top:0;border-top:none; }
    ...
    <div class="settings-section">
      <h3>✉️ E-Mail
        <span style="font-weight:400;font-size:11px;color:var(--text3);background:var(--bg3);padding:2px 8px;border-radius:10px;margin-left:6px;">Absender, Signatur & alle Textbausteine</span>
      </h3>

      <!-- Absender & Signatur -->
      <div class="settings-subsection">
        <h4>👤 Absender & Signatur</h4>
        <div class="form-grid">
      ...
      <div class="settings-subsection">
        <h4>📲 Terminbenachrichtigung
          <span style="font-weight:400;font-size:11px;color:var(--purple);background:rgba(139,92,246,0.12);padding:2px 8px;border-radius:10px;">geht sofort beim Eintragen raus</span>
        </h4>
      ...
      <!-- Terminbestätigung -->
      <div class="settings-subsection">
        <h4>📧 Terminbestätigung
          <span style="font-weight:400;font-size:11px;color:var(--accent);background:rgba(59,130,246,0.12);padding:2px 8px;border-radius:10px;">1 Tag vor dem Termin</span>
        </h4>
        <p style="font-size:13px;color:var(--text2);margin-bottom:14px;">
          Geht automatisch einen Tag vor dem Termin raus – inklusive .ics-Kalenderdatei im Anhang (Google, Apple, Outlook – der Empfänger klickt einmal drauf und der Termin steht im Kalender).<br/>
          <span style="font-size:12px;color:var(--text3);">Platzhalter: <code style="background:var(--bg3);padding:1px 5px;border-radius:4px;">{vorname}</code> <code style="background:var(--bg3);padding:1px 5px;border-radius:4px;">{nachname}</code> <code style="background:var(--bg3);padding:1px 5px;border-radius:4px;">{datum}</code> <code style="background:var(--bg3);padding:1px 5px;border-radius:4px;">{uhrzeit}</code> <code style="background:var(--bg3);padding:1px 5px;border-radius:4px;">{signatur}</code></span>
        </p>
        <div class="form-grid">
          <div class="field full"><label>Betreff</label>
      ...
      <div class="settings-subsection">
        <h4>🔔 Terminerinnerung
          <span style="font-weight:400;font-size:11px;color:var(--amber);background:rgba(245,158,11,0.12);padding:2px 8px;border-radius:10px;">am Termintag um 09:00 Uhr</span>
        </h4>
        <p style="font-size:13px;color:var(--text2);margin-bottom:14px;">
    ...
    <div class="settings-section">
      ...
      <div class="settings-subsection">
        <h4 style="color:#8b5cf6;">👥 Für Mitarbeiter</h4>
    ...
    <div class="settings-section">
      <h3>🔧 Technische Integrationen
        <span style="font-weight:400;font-size:11px;color:var(--text3);background:var(--bg3);padding:2px 8px;border-radius:10px;margin-left:6px;">API-Keys, OAuth & Versandtechnik</span>
      </h3>
      ...
      <!-- Outlook -->
      <div class="settings-subsection">
        <h4>📧 Outlook / Microsoft 365 API</h4>
        <p style="font-size:13px;color:var(--text2);margin-bottom:14px;">
    ...
    <div class="settings-section">
      ...
      <div style="display:flex;flex-direction:column;gap:12px;">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
          ...
          <button class="btn-ghost" id="copyWpLinkBtn" style="white-space:nowrap;">
  ...
  (function() {
    ...
    const copyBtn= document.getElementById('copyWpLinkBtn');
    ...
    if (copyBtn) {
      copyBtn.addEventListener('click', () => {
        const link = location.origin + location.pathname.replace(/[^/]*$/, '') + 'wochenplan-public.html';
        navigator.clipboard.writeText(link).then(() => {
          status.innerHTML = '<span style="color:var(--green)"><i class="ti ti-check"></i> Link kopiert!</span>';
...
/**
 * Macht alle .settings-section-Blöcke ein-/ausklappbar (Klick auf h3).
 * Idempotent: kann mehrfach aufgerufen werden (z.B. nachdem weitere
 * Settings-Sections per Patch nachträglich eingefügt wurden), ohne
 * Klick-Handler doppelt zu binden.
 */
function initSettingsCollapse() {
  const SETTINGS_COLLAPSE_KEY = 'settingsCollapsed_v1';
  let collapsedSettings;
  try { collapsedSettings = new Set(JSON.parse(localStorage.getItem(SETTINGS_COLLAPSE_KEY) || '[]')); }
  catch(e) { collapsedSettings = new Set(); }
  document.querySelectorAll('.settings-section').forEach(section => {
    const h3 = section.querySelector(':scope > h3');
    if (!h3) return;
    const key = h3.textContent.trim();
    if (collapsedSettings.has(key)) section.classList.add('collapsed');
    if (h3._collapseBound) return;
    h3._collapseBound = true;
    h3.addEventListener('click', () => {
      section.classList.toggle('collapsed');
      if (section.classList.contains('collapsed')) collapsedSettings.add(key);
      else collapsedSettings.delete(key);
      try { localStorage.setItem(SETTINGS_COLLAPSE_KEY, JSON.stringify([...collapsedSettings])); } catch(e) {}
    });
  });
}
...
// Mail-Texte/Signatur zusätzlich in Firestore spiegeln, damit das
// GitHub-Actions-Hintergrundskript (Terminbestätigung 1 Tag vorher,
// Erinnerung am Termintag) dieselben Texte wie die App verwenden kann.
async function syncEmailSettings() {
  const s = JSON.parse(localStorage.getItem('crmSettings') || '{}');
  try {
    const { setDoc, doc: fsDoc } = await import("https://www.gstatic.com/firebasejs/11.3.1/firebase-firestore.js");
    await setDoc(fsDoc(db, 'settings', 'email'), {
      gmailSender:        s.gmailSender        || 'nawin.telis@gmail.com',
      confirmSubject:     s.confirmSubject     || 'Terminbestätigung – {datum}',
      emailConfirmText:   s.emailConfirmText   || '',
      reminderSubject:    s.reminderSubject    || 'Erinnerung: Unser Termin heute um {uhrzeit} Uhr',
      emailReminderText:  s.emailReminderText  || '',
      notifySubject:      s.notifySubject      || 'Ihr Termin am {datum}',
      emailNotifyText:    s.emailNotifyText    || '',
      emailSig:           s.emailSig           || '',
      emailSigImage:      s.emailSigImage      || '',
    });
  } catch(e) {
    console.warn('Email-Settings sync fehlgeschlagen:', e.message);
  }
}
...
window.confirmPrivateSlot = async function(slotId) {
    ...
    <div style="background:var(--bg2);border:1px solid var(--border2);border-radius:18px;
      ...
      <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:18px;">
        <label style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;background:var(--bg3);border:1px solid var(--border2);border-radius:8px;cursor:pointer;transition:background .15s;"
          ...
          <input type="checkbox" id="pn_notify" checked style="width:16px;height:16px;accent-color:var(--purple);margin-top:1px;flex-shrink:0;" />
          <div>
            <div style="font-size:14px;font-weight:500;">📲 Terminbenachrichtigung <span style="font-size:10px;font-weight:600;color:var(--purple);background:rgba(139,92,246,0.15);padding:2px 7px;border-radius:8px;margin-left:4px;">SOFORT</span></div>
            <div style="font-size:11px;color:var(--text3);margin-top:2px;">Geht direkt beim Bestätigen raus – Termindetails mit Kalender-Datei (.ics)</div>
          </div>
        </label>
        <label style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;background:var(--bg3);border:1px solid var(--border2);border-radius:8px;cursor:pointer;transition:background .15s;"
          ...
          <div>
            <div style="font-size:14px;font-weight:500;">📧 Terminbestätigung <span style="font-size:10px;font-weight:600;color:#10b981;background:rgba(16,185,129,0.12);padding:2px 7px;border-radius:8px;margin-left:4px;">1 TAG VORHER</span></div>
            <div style="font-size:11px;color:var(--text3);margin-top:2px;">Automatisch 1 Tag vor dem Termin per E-Mail (via GitHub Actions)</div>
          </div>
        </label>
        <label style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;background:var(--bg3);border:1px solid var(--border2);border-radius:8px;cursor:pointer;transition:background .15s;"
          ...
          <div>
            <div style="font-size:14px;font-weight:500;">🔔 Terminerinnerung <span style="font-size:10px;font-weight:600;color:var(--amber);background:rgba(245,158,11,0.12);padding:2px 7px;border-radius:8px;margin-left:4px;">TERMINTAG 9 UHR</span></div>
            <div style="font-size:11px;color:var(--text3);margin-top:2px;">Am Termintag morgens um 09:00 Uhr automatisch per E-Mail</div>
          </div>
        </label>
      </div>
      <div style="display:flex;gap:8px;">
  ...
  document.getElementById('pn_confirm_btn').onclick = async () => {
    ...
    if (slot.telefon || slot.email) {
      ...
      if (contactObj.email) {
        // Alle Flags direkt per slotId setzen – kein Re-Query nötig
        try {
          const flagUpdate = {};
          if (sendConfirm)  flagUpdate.sendConfirmEmail = true;
          if (sendReminder) flagUpdate.sendReminder     = true;
          if (Object.keys(flagUpdate).length > 0) {
            await updateDoc(doc(db, 'callSlots', slotId), flagUpdate);
          }
        } catch(fe) { console.warn('Flag-Update fehlgeschlagen:', fe.message); }

        if (sendConfirm)  toast('📬 Bestätigung wird 1 Tag vor dem Termin automatisch per E-Mail versendet.', 'success');
        if (sendReminder) toast('🔔 Erinnerung am Termintag wird automatisch per E-Mail versendet.', 'success');

        if (sendNotify) {
          // Sofortige Benachrichtigungs-E-Mail mit Kalender-Einladung (.ics)
          await sendConfirmationEmail(contactObj, { ...slotObj, mailType: 'notify', withIcs: true });
        }
      } else if (sendConfirm && slot.telefon) {
        // Fallback WhatsApp wenn keine E-Mail vorhanden
  ...
  notifDlg.innerHTML = `
    <div style="background:var(--bg2);border:1px solid var(--border2);border-radius:18px;
      ...
      <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:18px;">
        ...
        <label style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;background:var(--bg3);border:1px solid var(--border2);border-radius:8px;cursor:pointer;transition:background .15s;"
          onmouseover="this.style.background='rgba(245,158,11,0.08)'" onmouseout="this.style.background='var(--bg3)'">
          <input type="checkbox" id="bn_reminder" checked style="width:16px;height:16px;accent-color:var(--amber);margin-top:1px;flex-shrink:0;" />
          <div>
        ...
        <label style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;background:var(--bg3);border:1px solid var(--border2);border-radius:8px;cursor:pointer;transition:background .15s;"
          onmouseover="this.style.background='rgba(139,92,246,0.08)'" onmouseout="this.style.background='var(--bg3)'">
          <input type="checkbox" id="bn_notify" checked style="width:16px;height:16px;accent-color:var(--purple);margin-top:1px;flex-shrink:0;" />
          <div>
...
function showLoginScreen() {
  ...
  document.getElementById('content').innerHTML = '';
  document.getElementById('pageTitle').textContent = 'Tracker';

  const loginDiv = document.createElement('div');
  loginDiv.id = 'login-screen';
  loginDiv.style.cssText = `
    position:fixed;inset:0;z-index:99999;background:var(--bg, #0f172a);
  ...
  loginDiv.innerHTML = `
    <div style="background:var(--bg2);border:1px solid var(--border2);border-radius:20px;
      ...
      <h2 style="font-size:22px;font-weight:700;margin-bottom:8px;color:var(--text);">Tracker</h2>
    ...
    try {
      ...
      if (credential?.accessToken) {
        const tokenData = JSON.stringify({
          ...
          expires: Date.now() + 3600000, // ~1h, wie bei Google OAuth-Tokens üblich
        });
        localStorage.setItem('gmailOAuthToken', tokenData);
        localStorage.setItem('gcalOAuthToken_' + result.user.email, tokenData);
      }
    } catch (e) {
...
onAuthStateChanged(auth, (user) => {
  ...
  if (user) {
    ...
    // Gmail-Token proaktiv alle 45 Min erneuern (läuft nach 60 Min ab).
    // Passiert still im Hintergrund solange der User eingeloggt ist –
    // kein Pop-up, kein erneutes Anmelden nötig.
    if (window._gmailTokenRefreshInterval) clearInterval(window._gmailTokenRefreshInterval);
    window._gmailTokenRefreshInterval = setInterval(async () => {
      try {
        const cached = JSON.parse(localStorage.getItem('gmailOAuthToken') || 'null');
        if (!cached) return; // noch nie angemeldet – nichts zu erneuern
        const timeLeft = cached.expires - Date.now();
        if (timeLeft > 10 * 60 * 1000) return; // noch mehr als 10 Min gültig – OK
        // Token läuft bald ab → still erneuern
        await getGmailToken();
        console.log('[Gmail] Token still erneuert.');
      } catch(e) { console.warn('[Gmail] Stille Token-Erneuerung fehlgeschlagen:', e.message); }
    }, 45 * 60 * 1000); // alle 45 Minuten

  } else {
    if (window._gmailTokenRefreshInterval) {
      clearInterval(window._gmailTokenRefreshInterval);
      window._gmailTokenRefreshInterval = null;
    }
    hideLogoutBtn();
    showLoginScreen();
  }
...
/**
 * Setzt/entfernt die "Potenzielle/r Mitarbeiter:in"-Markierung eines Kontakts.
 */
window.setFutureEmployee = async function(contactId, value) {
  try {
    await updateDoc(doc(db, 'contacts', contactId), { futureEmployee: !!value });
    const idx = contacts.findIndex(c => c.id === contactId);
    if (idx !== -1) contacts[idx].futureEmployee = !!value;
    toast(value ? 'Als potenzielle/r Mitarbeiter:in markiert!' : 'Markierung entfernt.');
  } catch {
    const idx = contacts.findIndex(c => c.id === contactId);
    if (idx !== -1) contacts[idx].futureEmployee = !!value;
    toast('Gespeichert (offline).');
...
function buildAltTemplateSettingsHTML(s) {
  ...
  return `
    ...
    <div class="settings-subsection" id="altTemplateSettingsSection">
      <h4><i class="ti ti-mail" style="margin-right:6px;"></i>E-Mail-Vorlagen \u2013 Alternativtermine</h4>
...
function _extRenderSettings() {
  ...
  requestAnimationFrame(() => {
    ...
    // ── E-Mail-Vorlagen DIREKT in den E-Mail-Bereich einfügen (als letzte Unterkategorie) ──
    if (!document.getElementById('altTemplateSettingsSection')) {
      // E-Mail-Sektion finden: enthält den s_emailReminderText textarea (Terminerinnerung)
      const reminderTextarea = document.getElementById('s_emailReminderText');
      const emailSection     = reminderTextarea?.closest('.settings-section');
      if (emailSection) {
        emailSection.insertAdjacentHTML('beforeend', buildAltTemplateSettingsHTML(s));
      } else {
...
// ── Vorlagen-State (Firestore-backed, localStorage nur als schneller Cache) ──
...
// Verbindet die lokalen Feld-IDs mit den Firestore-Feldnamen, die
// content.js (Extension, auf kleinanzeigen.de) über background.js erwartet.
// WICHTIG: Diese Feldnamen müssen exakt mit DEFAULT_TEMPLATES in content.js übereinstimmen.
const KA_VORLAGEN_FIRESTORE_KEYS = {
  ka_activeMessage:        'aktive',
  ka_passiveMessage:       'passive',
  ka_steuerPrivatMessage:  'mandanten_steuerberater_privat',
  ka_steuerFirmaMessage:   'mandanten_steuerberater_firma',
  ka_versicherungMessage:  'mandanten_versicherung',
  ka_kreditMessage:        'mandanten_kredit',
  ka_finanzberaterMessage: 'mandanten_finanzberater',
  ka_sonstigesMessage:     'mandanten_sonstiges',
};
...
// Beim Start einmal die aktuellsten Werte aus Firestore holen (falls z.B. von
// einem anderen Gerät/der Extension aus zuletzt gespeichert wurde) und die
// Textareas + den localStorage-Cache damit auffrischen.
async function kaLoadVorlagenFromFirestore() {
  try {
    const snap = await getDoc(doc(db, 'settings', 'kaVorlagen'));
    if (!snap.exists()) return;
    const remote = snap.data() || {};
    for (const [localId, fsKey] of Object.entries(KA_VORLAGEN_FIRESTORE_KEYS)) {
      if (remote[fsKey] === undefined) continue;
      localStorage.setItem('kaVorlage_' + localId, remote[fsKey]);
      const el = document.getElementById(localId);
      if (el) el.value = remote[fsKey];
    }
  } catch (e) {
    console.warn('[KA] Vorlagen aus Firestore laden fehlgeschlagen:', e.message);
  }
}
kaLoadVorlagenFromFirestore();
...
window.kaSaveVorlage = function(id) {
  ...
  try {
    ...
    const fsKey = KA_VORLAGEN_FIRESTORE_KEYS[id];
    if (fsKey) {
      setDoc(doc(db, 'settings', 'kaVorlagen'), { [fsKey]: el.value }, { merge: true })
        .then(() => window.toast?.('Vorlage gespeichert · live in Extension aktiv ✓', 'success'))
        .catch(e => window.toast?.('In Firestore gespeichert fehlgeschlagen: ' + e.message, 'error'));
    } else {
...
window.kaResetVorlage = function(id) {
  try {
    ...
    const fsKey = KA_VORLAGEN_FIRESTORE_KEYS[id];
    if (fsKey) {
      setDoc(doc(db, 'settings', 'kaVorlagen'), { [fsKey]: KA_VORLAGEN_DEFAULTS[id] || '' }, { merge: true })
        .catch(e => console.warn('[KA] Reset in Firestore fehlgeschlagen:', e.message));
    }
...
function kaRenderSocialMedia() {
  ...
  const kanalBlocks = KA_SOCIAL_CHANNELS.map(k => {
    ...
    const msgs     = kaSocialMessages.filter(m => (m.kanal || '').toLowerCase() === k.toLowerCase());
    ...
    const msgCards = !checked ? '' : msgs.length === 0
      ...
      : msgs.map(msg => {
          ...
          // Display-Name bestimmen
          const displayName = (msg.contactName && !msg.contactName.startsWith('http'))
            ? msg.contactName
            : (msg.absender && !msg.absender.startsWith('http') ? msg.absender : '(Unbekannt)');

          // Texte die NICHT als Nachricht angezeigt werden sollen
          const skipMsgTexts = new Set([
            displayName, msg.contactName, msg.absender,
            'Instagram', 'TikTok', 'WhatsApp', 'LinkedIn', 'Xing', 'Facebook',
            displayName + ' · Instagram', displayName + ' · TikTok',
            msg.contactName + ' · Instagram', msg.absender + ' · Instagram',
            'Profil ansehen', 'Profil anzeigen',
          ].filter(Boolean).map(t => t.trim()));
          const dateRxMsg = /^\d{1,2}\.\d{2}\.\d{4}/;
          const timeRxMsg = /^(vor\s+\d+|heute|gestern|\d+\s*(min|std|tag|woch|monat|jahr|sec))/i;

          // Gültige Nachrichten filtern
          const cleanMsgs = (Array.isArray(msg.messages) ? msg.messages : [])
            .filter(m => {
              const t = (m.text || '').trim();
              if (!t || t.length < 1) return false;
              if (skipMsgTexts.has(t)) return false;
              if (dateRxMsg.test(t)) return false;
              if (timeRxMsg.test(t)) return false;
              return true;
            })
            .sort((a, b) => {
              // Nach top sortieren falls vorhanden (DOM-Position: oben=alt, unten=neu)
              if (a.top != null && b.top != null) return a.top - b.top;
              return 0; // sonst Reihenfolge beibehalten
            })
            .map(({ top, ...m }) => m);

          // Chatverlauf als Bubbles
          const verlauf = cleanMsgs.length > 0
            ? cleanMsgs.map(m => {
                ...
                return `<div style="display:flex;flex-direction:column;align-items:${isMe ? 'flex-end' : 'flex-start'};margin-bottom:6px;">
                  <div style="max-width:75%;padding:8px 12px;border-radius:${isMe ? '16px 16px 4px 16px' : '16px 16px 16px 4px'};
                    background:${isMe ? col.c : 'var(--bg2,var(--card-bg))'};
                    color:${isMe ? '#fff' : 'var(--text,var(--text-color))'};
                    font-size:13px;line-height:1.5;word-break:break-word;">
                    ${m.text}
                  </div>
                  <span style="font-size:10px;color:var(--text3,var(--text-muted));margin-top:2px;padding:0 2px;">
                    ${isMe ? 'Du' : displayName}
                  </span>
                </div>`;
              }).join('')
            : `<div style="font-size:12px;color:var(--text3,var(--text-muted));text-align:center;padding:8px 0;">
                Öffne den Chat auf ${k} um Nachrichten zu laden
              </div>`;

          const verlaufId = 'verlauf_' + msg.id;
          ...
          return `
            <div style="margin:0 16px 8px 50px;background:var(--bg3, var(--card-bg));border:1px solid var(--border, var(--border-color));
              ...
              <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;flex-wrap:wrap;">
                <span style="font-weight:600;font-size:14px;color:var(--text, var(--text-color));">${displayName}</span>
                ...
                ${msg.profilLink ? `<a href="${msg.profilLink}" target="_blank" style="font-size:11px;color:var(--text3,var(--text-muted));text-decoration:none;margin-left:auto;">🔗 Profil</a>` : ''}
              </div>
              <div id="${verlaufId}" style="max-height:280px;overflow-y:auto;padding:10px;border-radius:10px;
                background:var(--surface,var(--card-bg));margin-bottom:10px;
